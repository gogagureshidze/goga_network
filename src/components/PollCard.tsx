...
interface PollCardProps {
  poll: Poll;
  postId: number;
  onVoteSuccess?: (postId: number) => void;
}
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (result.error) {
          ...
          // Success! Keep the optimistic vote and refresh the post
          ...
          return (
            <div key={option.id} className="flex flex-col gap-2">
              <button
                ...
                disabled={hasVoted || isExpired || isPending || !user}
                className={`relative overflow-hidden rounded-xl border-2 transition-all duration-300 ${
                  isSelected
                    ? "border-rose-500 dark:border-gray-500 shadow-lg scale-105"
                    : hasVoted || isExpired
                    ? "border-orange-200 dark:border-gray-700"
                    : "border-orange-300 dark:border-gray-600 hover:border-rose-400 dark:hover:border-gray-500 hover:scale-102 hover:shadow-md active:scale-98"
                } ${
                  !hasVoted && !isExpired && user
                    ? "cursor-pointer"
                    : "cursor-default"
