    ...
    <>
      ...
      >
        ...
        <div className="p-3">
          ...
          {hasMedia && (
            <div
              className={`grid gap-1.5 rounded-xl overflow-hidden mb-3 ${postData.media!.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
            >
          ...
          {isEvent && postData.event && (
            <div className="relative overflow-hidden rounded-2xl border border-white/40 dark:border-white/10 bg-gradient-to-br from-blue-500/5 to-cyan-500/10 dark:from-blue-500/10 dark:to-cyan-500/5 backdrop-blur-2xl p-3 mb-3 shadow-sm group">
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-blue-500/20 dark:bg-blue-400/20 blur-[32px] rounded-full pointer-events-none transition-transform duration-700 group-hover:scale-150" />
          ...
          {isPoll && postData.poll && (
            <div className="relative overflow-hidden rounded-2xl border border-white/40 dark:border-white/10 bg-gradient-to-br from-orange-500/5 to-rose-500/10 dark:from-orange-500/10 dark:to-rose-500/5 backdrop-blur-2xl p-4 mb-3 shadow-sm group">
              <div className="absolute -top-12 -right-4 w-32 h-32 bg-orange-500/20 dark:bg-orange-500/30 blur-[32px] rounded-full pointer-events-none transition-transform duration-700 group-hover:scale-150" />
              <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-rose-400/20 dark:bg-rose-500/20 blur-[32px] rounded-full pointer-events-none" />
                ...
                <div className="space-y-2">
                  {postData.poll.options.slice(0, 2).map((option) => (
                    ...
                    >
                      ...
                      <span className="text-[11px] font-bold text-orange-700 dark:text-orange-300 bg-white/60 dark:bg-slate-800/60 border border-white/30 dark:border-white/5 px-2 py-1 rounded-md shadow-inner z-10 shrink-0">
                        {option.votesCount || 0}
                      </span>
                    </div>
                  ))}
          ...
          {/* Stats (snapshot — open modal for live counts) */}
          ...
          {/* View Post → opens modal with live fresh data */}
          <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-700/50">
            ...
            >
              <div className="absolute inset-0 bg-white/20 dark:bg-white/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      ...
      {/* Modal fetches fresh data using only the id */}
      {showModal && (
        <PostModal postId={postData.id} onClose={() => setShowModal(false)} />
      )}
    </>
  );
}
